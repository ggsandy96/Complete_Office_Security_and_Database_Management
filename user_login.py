...
counter = 0
def ch():
	os.system('python /home/linaro/project/user_login_page2.py &')
	close(master)
...
def show_entry_fields():
	...
	for i in range(0,a):
		if(usr == data[i][0]):
			...
			if(pwd == data[i][1]):
				...
				cmd =  "INSERT INTO log (username,log_time,result) values('"+ b.get('name') +"','" + b.get('log_time') + "','success','employee')"
				db = MySQLdb.connect("localhost","root","root","testdb")
				os.system("ps -ef | grep 'motion.py' | awk '{print $2}' | xargs sudo kill -USR2")
				cursor = db.cursor()
				lab=""
				cursor.execute(cmd)
				db.commit()
				db.close()
				cursor.close()
				Button(master, text='change password', command=ch).grid(row=6, column=4)
				...
				log_time= str(now)
				b={'name':usr,'log_time':log_time}
				cmd =  "INSERT INTO log (username,log_time,result) values('"+ b.get('name') +"','" + b.get('log_time') + "','incorrect','employee')"
				db = MySQLdb.connect("localhost","root","root","testdb")
				cursor = db.cursor()
				cursor.execute(cmd)
				db.commit()
	...
	if(s != 0):
		...
		b={'name':usr,'log_time':log_time}
		cmd =  "INSERT INTO log (username,log_time,result) values('"+ b.get('name') +"','" + b.get('log_time') + "','failed','employee')"
		db = MySQLdb.connect("localhost","root","root","testdb")
		cursor = db.cursor()
		cursor.execute(cmd)
		db.commit()
...
a1=Button(master, text='user login').grid(row=2, column=4)
