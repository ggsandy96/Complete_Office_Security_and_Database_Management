...
counter = 0
def ch():
	os.system('python /home/linaro/project/admin_login_page2.py &')
	close(master)
...
def show_entry_fields():
	...
	cursor = db.cursor()
	cursor.execute ("select username, password from admin_login")
	...
	for i in range(0,a):
		if(usr == data[i][0]):
			...
			if(pwd == data[i][1]):
				...
				cmd =  "INSERT INTO log (username,log_time,result,type) values('"+ b.get('name') +"','" + b.get('log_time') + "','success','admin')"
				db = MySQLdb.connect("localhost","root","root","testdb")
				cursor = db.cursor()
				lab=""
				cursor.execute(cmd)
				...
				log_time= str(now)
				b={'name':usr,'log_time':log_time}
				cmd =  "INSERT INTO log (username,log_time,result) values('"+ b.get('name') +"','" + b.get('log_time') + "','incorrect','admin')"
				db = MySQLdb.connect("localhost","root","root","testdb")
				cursor = db.cursor()
				cursor.execute(cmd)
				db.commit()
	...
	if(s != 0):
		...
		b={'name':usr,'log_time':log_time}
		cmd =  "INSERT INTO log (username,log_time,result) values('"+ b.get('name') +"','" + b.get('log_time') + "','failed','admin')"
		db = MySQLdb.connect("localhost","root","root","testdb")
		cursor = db.cursor()
		cursor.execute(cmd)
		db.commit()
...
b=Button(master)
photo1=PhotoImage(file='/home/linaro/project/admin_login.gif')
